...
describe('<Portal />', function() {
    ...
    it('displays the portal and renders its children', async function() {
        const {driver: portal} = clientRenderer.render(
            <Portal>
                <span>Portal Body</span>
            </Portal>).withDriver(PortalTestDriver);

        await waitFor(() => {
            expect(portal.portalRoot).to.be.present();
    ...
    it('applies supplied styles to the popup and updates them if changed', async function() {
        ...
        await waitFor(() => expect(portal.portalRoot).to.have.nested.property('style.position', 'absolute'));
    ...
    it('removes the component when unmounting', async function() {
        ...
        await waitFor(() => expect(portal.portalRoot).to.be.present());
    ...
    it('renders the portal in the bottom of the DOM', async function() {
        ...
        await waitFor(() => {
            ...
            expect(portal.portalRoot.compareDocumentPosition(portal.content[0]) & Node.DOCUMENT_POSITION_CONTAINED_BY,
                'children contained in portal').to.equal(Node.DOCUMENT_POSITION_CONTAINED_BY);
            expect(container.compareDocumentPosition(portal.portalRoot) & Node.DOCUMENT_POSITION_FOLLOWING,
    ...
    it('renders with a className passed as a prop', async function() {
        ...
        await waitFor(() => expect((portal.portalRoot as Element).className).to.contain('test-class'));
    });
});
