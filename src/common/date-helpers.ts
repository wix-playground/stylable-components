export function getDayNames (startingDay: number = 0, nameLength: string = 'short', locale: string = 'en-US'): Array<string> {
    ...
    for (let day = 0; day < 7; day++) {
        let dayName = new Date(Date.UTC(1970, 5, day + startingDay, 0, 0, 0)).toLocaleString(locale, { weekday: nameLength });
...
export function getMonthNames (locale: string = 'en-US'): Array<string> {
    ...
    for (let month = 0; month < 12; month++) {
        let monthName = new Date(Date.UTC(1970, month, 1, 0, 0, 0)).toLocaleString(locale, { month: 'long' });
...
export function getMonthFromOffset (date: Date, offset: number): Date {
    return new Date(Date.UTC(date.getFullYear(), date.getMonth() + offset, 1, 0, 0, 0));
}
...
export function getDaysInMonth (date: Date): number {
    ...
    return new Date(Date.UTC(date.getFullYear(), date.getMonth() + 1, 0, 0, 0, 0)).getDate();
}

export function getLastDayOfMonth (date: Date): number {
    return new Date(Date.UTC(date.getFullYear(), date.getMonth() + 1, 0, 0, 0, 0)).getDay();
}

export function getNumOfPreviousDays (date: Date, startingDay: number = 0): number {
    // Days start from Sunday (Sunday = 0, Monday = 1, etc.)
    const previousDays: number = (new Date(Date.UTC(date.getFullYear(), date.getMonth(), 1, 0, 0, 0)).getDay() - startingDay) + 7;
