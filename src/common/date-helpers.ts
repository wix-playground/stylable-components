export function getDayNames (startingDay: number = 0, nameLength: string = 'short', locale: string = 'en-US'): Array<string> {
    ...
    for (let day = 0; day < 7; day++) {
        const dayName = new Date(Date.UTC(1970, 5, day + startingDay)).toLocaleString(locale, { weekday: nameLength });
...
export function getMonthNames (locale: string = 'en-US'): Array<string> {
    ...
    for (let month = 0; month < 12; month++) {
        const monthName = new Date(Date.UTC(1970, month, 1)).toLocaleString(locale, { month: 'long' });
...
export function getLastDayOfMonth (date: Date): number {
    return new Date(Date.UTC(date.getFullYear(), date.getMonth() + 1, 0)).getDay();
}

export function getNumOfPreviousDays (date: Date, startingDay: number = 0): number {
    // Days start from Sunday (Sunday = 0, Monday = 1, etc.)
    const previousDays: number = (new Date(Date.UTC(date.getFullYear(), date.getMonth(), 1)).getDay() - startingDay) + 7;
